...
class FolderApp:
    def __init__(self, master):
        ...
        master.tk.call('source', self.dsc + '/resources/themes/awdark.tcl')
        ...
        master.iconbitmap(os.path.join(self.dsc, "resources", "image", "icon.ico"))

        self.style.configure('TButton', justify="left", anchor='w')
        self.background_image = PhotoImage(file=os.path.join(self.dsc, "resources", "image", "background.png"))
    ...
    def json_setting(self, status = "read", key = None, value = None):
        if status == "start":
            ...
            with open("resources/setting.json", 'r') as settings:
    ...
    def play_sound_on_start_demon(self):
        if self.konfiguracja_programu["volume"] != 0:
            mixer.music.load(os.path.join(self.dsc, "resources", "sounds", self.konfiguracja_programu["start_sound"]))
            mixer.music.set_volume(self.konfiguracja_programu["volume"])
            mixer.music.play()

    def error_sound_demon(self):
        if self.konfiguracja_programu["volume"] != 0:
            mixer.music.load(os.path.join(self.dsc, "resources", "sounds", self.konfiguracja_programu["error_sound"]))
            mixer.music.set_volume(self.konfiguracja_programu["volume"])
            mixer.music.play()
    ...
    def cena_ilosc_window(self):
        ...
        self.window.iconbitmap(os.path.join(self.dsc, "resources", "image", "icon.ico"))
    ...
    def button_back_pack(self, frame, commend = "main"):
        ...
        self.dodaj_button = self.utworz_przycisk(frame, "button_back_pack", commend, side='bottom', padx=5, pady=5)
