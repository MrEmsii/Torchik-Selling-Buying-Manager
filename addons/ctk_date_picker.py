...
class CTkDatePicker(ctk.CTkFrame):
            ...
            for day_col in range(7):
                if week == 2 and day_col < start_day:
                    ...
                    if ctk.get_appearance_mode() == "Light":
                        btn = ctk.CTkButton(self.calendar_frame, text=str(day), width=3, command=lambda day=day: self.select_date(year, month, day), fg_color="transparent", text_color="black", hover_color="#3b8ed0")
                    else:
                        btn = ctk.CTkButton(self.calendar_frame, text=str(day), width=3, command=lambda day=day: self.select_date(year, month, day), fg_color="transparent")
    ...
    def select_date(self, year = datetime.now().year, month= datetime.now().month, day= datetime.now().day):
        """
        ...
        self.selected_date = datetime(year=year, month=month, day=day)
